...
Time to first events were presented with cumulative incidence curves using the Kaplan-Meier estimate for the outcome death and the Aalen-Johansen estimate, considering death as a competing event, for the other outcomes. The mean cumulative function was used to depict the recurrent events by EF och CCI. 
...
medcci <- rsdata %>%
  summarise(
    med = fn(median(sos_com_charlsonciage), dig = 0),
    q1 = fn(quantile(sos_com_charlsonciage, probs = 0.25), dig = 0),
    q3 = fn(quantile(sos_com_charlsonciage, probs = 0.75), dig = 0)
  ) %>%
  mutate(out = paste0(med, " (", q1, "-", q3, ")")) %>%
  pull(out)
...
The median (q1-q3) age is `r med` and `r percfem`% females. The median (q1-q3) ACCI is `r medcci`. 
