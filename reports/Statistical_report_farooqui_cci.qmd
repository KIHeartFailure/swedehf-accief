...
Please specify what time periods to look at SGLT2 in ref, mref and pef respectively. 

Please specify target doses (see e-mail). 
...
Cox proportional hazards regressions were used to model the time to first event. Recurrent events were model using a negative binomial regression including the 
log of time as an offset in the model. 

Data were censored at `r global_followup_year` years ?????SHOULD use compelte follow-up (up to 22 years) or should use max X years???? follow-up, 2023-12-31 or emigration from Sweden. 
...
Please check variables in model and tables carefully. Should anything be added? Should age be adjusted for in model?
