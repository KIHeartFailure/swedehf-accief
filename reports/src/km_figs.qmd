...
kmfunc <- function(efcats = c("HFrEF", "HFmrEF", "HFpEF"), time, event, eventname, rep, xvar = "sos_com_charlsonciage_cat",
                   labposadj = ifelse(rep, c(0.08, 0.08, 0.08), c(0.05, 0.05, 0.05))) {
  data <- rsdata %>%
  ...
  # cox results
  # cox <- paste0(
  #  paste0(str_remove_all(cox[, "Model"], ", p-value"), ": "),
  #  cox[, "Previous cancer"]
  # )
  # cox <- paste0(cox, collapse = "\n")
  # cox <- str_replace_all(cox, "Hazard Ratio", "HR")
  # cox <- str_replace_all(cox, "Rate Ratio", "RR")
  ...
  if (rep) {
    fit <- mcf(formula(paste0("Recur(sos_outtime, lopnr, sos_out_hosphf == 'Yes') ~ ", xvar)), data = repdata)
...
#| cache: true
#| cache.comments: false
#| dependson: kmfunc, outcomesfunc, tbl-outcomes
#| fig-cap: "Cumulative incidence/Mean cumulative function"
#| fig-subcap: !expr outvars$name[c(1, 5, 7)]
#| layout-ncol: 2
#| layout-nrow: 3
...
#| cache: true
#| cache.comments: false
#| dependson: kmfunc, outcomesfunc, tbl-outcomes-ref
...
#| cache: true
#| cache.comments: false
#| dependson: kmfunc, outcomesfunc, tbl-outcomes-pef
