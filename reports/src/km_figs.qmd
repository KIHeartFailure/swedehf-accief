...
kmfunc <- function(efcats = c("HFrEF", "HFmrEF", "HFpEF"), time, event, eventname, type, xvar = "sos_com_charlsonciage_cat",
                   labposadj = ifelse(type == "rep", c(0.08, 0.08, 0.08), c(0.05, 0.05, 0.05)), repymax = 1.5, repdata = NULL) {
  data <- rsdata %>%
  ...
  if (type == "cr") {
    event <- paste0(event, "_cr")
    
    fit <- survfit(formula(paste0("Surv(", time, ",", event, ") ~ ", xvar)),
      data = data
    )
    s <- summary(fit, times = seq(0, global_followup, 1), extend = T)
    plot_data <- tibble(
  ...
  p <- ggplot(plot_data, aes(x = time / 365, y = estimate, color = strata)) +
    ...
    geom_text(
      ...
      nudge_x = 0, nudge_y = labposadj, size = 6.5, hjust = 0, 
      fontface = "bold"
    ) # +
  ...
  t <- ggplot(table_data, aes(y = strata, x = time / 365)) +
    geom_text(aes(label = n.risk), size = 6.5, fontface = "bold") +
    # scale_x_continuous(breaks = seq(0, endfollow, 1), limits = c(0, endfollow), expand = c(0, 0)) +
...
#|  - "HF hospitalization"
#|  - "Composite CV death or First HF hospitalization"
#| layout-ncol: 2
#| layout-nrow: 3
