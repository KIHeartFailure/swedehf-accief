...
crudemod <- summary(crudemodraw <- multinom(shf_ef_cat ~ sos_com_charlsonciage_cat, data = rsdata, trace = F))
crude <- tibble(
  type = "Crude",
...
adjmod <- with(imprsdata, multinom(formula(paste0("shf_ef_cat ~ sos_com_charlsonciage_cat + ", paste(modvarstmp, collapse = " + "))), trace = F))
adjmod <- summary(pool(adjmod))
adj <- tibble(
  type = "Adjusted",
  ef = rep(levels(rsdata$shf_ef_cat)[2:3], each = length(adjmod$term) / 2),
