...
vars <- vars %>%
  mutate(
    ...
    Variable = str_replace_all(Variable, fixed("First HF hospitalization"), "Heart failure hospitalization"),
    ...
    Code = if_else(Variable == "Non-CV death", "ICD-10 All exept I, J81, K761, R570, G45", Code),
    Code = if_else(Variable == "All-cause hospitalization", NA_character_, Code),
  )
...
vars <- vars %>%
  mutate(
    ...
    footnote1 = if_else(Variable == "Charlson Comorbidity Index age-adjusted", footnote_marker_alphabet(1, format = "latex"), ""),
    footnote2 = if_else(str_detect(var, "sos_com") &
      !Variable %in% c("Charlson Comorbidity Index age-adjusted"),
    footnote_marker_alphabet(2, format = "latex"), ""
    ),
    footnote3 = if_else(str_detect(var, "hosp") & str_detect(Variable, "All-cause", negate = T), footnote_marker_alphabet(3, format = "latex"), ""),
    footnote4 = if_else(str_detect(var, "death") & str_detect(Variable, "All-cause", negate = T), footnote_marker_alphabet(4, format = "latex"), "")
  ) %>%
...
default_kable(vars,
  ...
  footnote(
    ...
    general_title = "",
    general = c(
      "Abbreviations: SwedeHF, The Swedish Heart Failure Registry; NPR, The National Patient Register; LISA, Longitudinal integrated database for health insurance and labour market studies; RTP, Register of the Total Population"
    ),
