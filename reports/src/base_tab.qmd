...
tab <- rsdata %>%
  ...
  select(var, level, pmis, levels(rsdata %>% pull(shf_ef_cat)), p)

tabtmp <- sapply(tabvars,
  FUN = basetabfunc_inner, data = rsdata,
  stratavar = "shf_ef_cat", catdig = catdig, contdig = contdig
)
...
cn <- c("Variable", "Missing (%)", levels(rsdata %>% pull(shf_ef_cat)), "p-value")
