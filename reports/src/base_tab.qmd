...
basetabfunc <- function(data, novars = NULL) {
  tab <- data %>%
    ...
    pivot_wider(names_from = sos_com_charlsonciage_cat, values_from = n) %>%
    mutate(
      var = "N (%)",
      level = NA,
      p = NA,
    ...
    select(var, level, pmis, levels(data %>% pull(sos_com_charlsonciage_cat)), p)

  tabvarstmp <- setdiff(tabvars, novars)
  tabtmp <- sapply(tabvarstmp,
    FUN = basetabfunc_inner, data = data,
    stratavar = "sos_com_charlsonciage_cat", catdig = catdig, contdig = contdig
  )
  ...
  cn <- c("Variable", "Missing (%)", levels(data %>% pull(sos_com_charlsonciage_cat)), "p-value")
  ...
  default_kable(tab,
    ...
    add_header_above(c(" " = 1, " " = 1, "Charlson Comorbidity Index" = 3, " " = 1)) %>%
    footnote(
...
basetabfunc(data = rsdata %>% filter(shf_ef_cat == "HFrEF"), novars = "shf_ef_cat")
```
...
basetabfunc(data = rsdata %>% filter(shf_ef_cat == "HFmrEF"), novars = "shf_ef_cat")
```
...
basetabfunc(data = rsdata %>% filter(shf_ef_cat == "HFpEF"), novars = "shf_ef_cat")
```
